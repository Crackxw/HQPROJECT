// StaticSound.cpp: implementation of the CStaticSound class.
//
//////////////////////////////////////////////////////////////////////
...
#ifdef _MT
CS_OBJECTS CStaticSound::m_CSObjects;
#endif
...
CStaticSound::CStaticSound(IPlayer* pPlayer) : m_nSndHandle(0),
											   m_nBytePerSec(0),
											   m_lVolume(0),
											   m_pPlayer(pPlayer)
{
...
//-----------------------------------------------------------------------------
BOOL CStaticSound::Open(const char* szFileName) 
{
...
BOOL CStaticSound::OpenSoundData(ISoundData* pSoundData, const char* szFileName)
{
	assert(pSoundData);
...
//-----------------------------------------------------------------------------
void CStaticSound::WriteWavDataToPlayerSoundBuffer(const int nPos, const int nSize, ISoundData* const pSoundData) const
{
	assert(m_pPlayer);
...
void CStaticSound::Play(BOOL bLoop) 
{
	assert(m_pPlayer);
...
void CStaticSound::Pause() const
{
	assert(m_pPlayer);
	m_pPlayer->Pause(m_nSndHandle);
}

void CStaticSound::Close() const
{
	assert(m_pPlayer);
	m_pPlayer->Close(m_nSndHandle);
}

int CStaticSound::GetTick() const
{
	assert(m_pPlayer);
...
void CStaticSound::SetVolume(const long lVolume)
{
	assert(m_pPlayer);
	m_lVolume = lVolume;
...
//-----------------------------------------------------------------------------
#ifdef _MT
void CStaticSound::LockThread(const int nIndex) 
{
	assert(nIndex < CS_MAX);
	EnterCriticalSection(&m_CSObjects.sCriticalSection[nIndex]);
}

void CStaticSound::UnLockThread(const int nIndex) 
{
	assert(nIndex < CS_MAX);
