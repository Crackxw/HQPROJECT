...
void XServerMgr::SendPacket( WORD servercode, char *pPacket, int len )
{

	BEGIN_JXLIST_LOOP( local_uiTemp, this, XServer, local_pServer )

		if( local_pServer->m_servercode == servercode ) {
			local_pServer->GetSocket()->SendPacket( pPacket, len );
			break;
		}
...
void XServerMgr::SendPacket( WORD servercode, JXPacket *pPacket )
{
	BEGIN_JXLIST_LOOP( local_uiTemp, this, XServer, local_pServer )
