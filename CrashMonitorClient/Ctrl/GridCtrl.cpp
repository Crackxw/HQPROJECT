...
//  MFC Grid Control
//
// Produced   by    Written by Chris Maunder (Ver 1.12a  1999.04.15)
//==================================================================
...
#define TEST_MESSAGE 90001

/////////////////////////////////////////////////////////////////////////////
// CGridCtrl

CGridCtrl   *g_pGridCtrl = NULL;
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage, LPCTSTR szData, LPCTSTR a_szStr )
{
    if (!IsWindow(m_hWnd))
        return 0;
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	{	
		switch( nChar )
		{
			...
			pOwner->SendMessage( USEWRONGCHAR, 0, 0L );
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult) 
{
 ...
 // SendMessageToParent(pgvItem->row, pgvItem->col, TEST_MESSAGE);
...
void CGridCtrl::Print() 
{
    ...
    CString strTitle;
	GetWindowText(strTitle);					// JHC Edit
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
	...
	GetWindowText(strHeaderString);				// JHC Edit
    ...
    // print parent window title in the centre (Gert Rijs)		// JHC Edit
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
	...
	if (strCurrent != str)
    {
		...
		SendMessageToParent(nRow, nCol, TEST_MESSAGE, str, strCurrent );
	}
}
...
/*
	CMenu tempMenu, *pMenu;
	CCellID cID = GetCellFromPt(point);

	if( cID.col == 0 && cID.row != 0){
		tempMenu.LoadMenu( IDR_MAINFRAME );
		pMenu = tempMenu.GetSubMenu(5);
		ClientToScreen(&point);
		pMenu->TrackPopupMenu( TPM_LEFTALIGN, point.x, point.y, this );
		m_nRowNumber = cID.row;
	}
*/
