/************************************************************************************************
	だ橾貲 : OnlineVillageStruct-Barrack.cpp
	氬渡濠 : 檜彌戮
...
//	撲貲	:	儅撩濠.
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
OnlineVillageStructureBarrack::OnlineVillageStructureBarrack()
{
	m_NowWork = BARRACK_NOWWORK_BUY_START;
	m_Say     = 0;

	// Image
	ZeroMemory(&m_Image_MainInterface_Back1,    sizeof(XSPR));
	ZeroMemory(&m_Image_CheckInterface,         sizeof(XSPR));
	ZeroMemory(&m_Image_Line_Check_Button,		sizeof(XSPR));
	...
	// Solder Data
	ZeroMemory(m_SolderList,          sizeof(OnInnBuySolderInfo) * MAX_BUYSOLDER_LIST);
	m_MaxSelectSolder = 0;
	m_NowSelectSolder = 0;
...
BOOL	OnlineVillageStructureBarrack::Action()
{
	return FALSE;
}
...
//	撲貲	:	蟾晦��
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
VOID	OnlineVillageStructureBarrack::Init(UI16 VillageCode, cltOnlineWorld *pOnlineWorld, VillageStructManager	*pVillageStructManager)
{	
	Free();
	...
	siKind	          = ON_VIL_ST_KIND_BARRACK;

	pMyOnlineWorld->pOnlineClient->SendRequestBarrackEnter();
}

//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
//	撲貲	:	п薯.
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
VOID	OnlineVillageStructureBarrack::Free()
{
	...
	if(m_Image_Line_Check_Button.Image)	   {clGrp.FreeXspr(m_Image_Line_Check_Button);	   ZeroMemory(&m_Image_Line_Check_Button.Image,sizeof(XSPR));}	
...
//	撲貲	:	擋暮.	
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
BOOL	OnlineVillageStructureBarrack::Poll(LPDIRECTDRAWSURFACE7 pSurface)
{
	char			TempBuffer[256];
	SI32			TempCount;
	SI32			TempUnitCount;
	...
	switch(m_NowWork)
	{
		case BARRACK_NOWWORK_BUY_START:
			pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_NONE, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), pMyOnlineWorld->pOnlineText->Get(ON_TEXT_WAIT));
			pMyOnlineWorld->pOnlineMsgBox->Draw(pSurface);
			break;

		case BARRACK_NOWWORK_BUY_STARTPOOL:
			Draw_Select(pSurface);
			pMyOnlineWorld->pOnlineMsgBox->Draw(pSurface);

			// 橾薑 衛除檜 雖釭賊 Pool 煎 檜翕
			m_NowWork        = BARRACK_NOWWORK_BUY_POOL;

			if(pMyOnlineWorld->pOnlineVillage->GetNation() == ON_COUNTRY_CHINA)
					PushEffect(EFFECT_BARRACK_C, 0, 0);
			break;

		case BARRACK_NOWWORK_BUY_POOL:
			...
			else
			{
				...
				if(m_Button_Buy.Process(TempbMouseDown) == TRUE)
				{
					...
					if(CheckLevel(m_SolderList[m_NowSelectSolder].siLevel, m_SolderList[m_NowSelectSolder].siCredit))
					{
						...
						if(TempUnitCount <= 0)
						{
							pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_OK, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK_NOSOLIDER));
						}
						else
						{
							SI16		MaxFollower;
							...
							if((ON_MAX_FOLLOWER_NUM - TempUnitCount) > MaxFollower)
							{
								...
								pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_OK, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), (char*)TempBuffer);
							}
							else
							{
								if(pMyOnlineWorld->pMyData->GetMoney() < m_SolderList[m_NowSelectSolder].siMoney)
								{
									pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_OK, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK_NOMONEY));
								}
								else
								{
									m_NowWork = BARRACK_NOWWORK_BUYCHECK_POOL;
								}
							}
						}
					}
					...
					{
						pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_OK, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK_SOLIDEREMPOLY), m_SolderList[m_NowSelectSolder].siLevel, m_SolderList[m_NowSelectSolder].siCredit);
					}
				}
					...
					for(TempCount1 = 0; TempCount1 < 6; TempCount1++)
					{
						for(TempCount2 = 0; TempCount2 < 2; TempCount2++)
						{
							...
							if(TempCount < m_MaxSelectSolder)
							{
								...
								if((ptMouse.x >= TempXPos) && (ptMouse.x < (TempXPos + 60)) && (ptMouse.y >= TempYPos) && (ptMouse.y < (TempYPos + 60)))
								{
...
END_HELP:
		...
		case BARRACK_NOWWORK_BUYCHECK_POOL:
			m_Button_Buy.SetAction(FALSE);
...
/*				if(GetAsyncKeyState(VK_RETURN))
				{
					...
					pMyOnlineWorld->pOnlineClient->SendRequestBarrackBuySolder(m_SolderList[m_NowSelectSolder].siCode, (BYTE*)TempBuffer);

					m_NowWork = BARRACK_NOWWORK_BUYCHECK_POOL_RETURNWAIT;
				}
				else
				{
*/
...
//				}
		...
		case BARRACK_NOWWORK_BUYCHECK_POOL_RETURNWAIT:
			pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_NONE, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), pMyOnlineWorld->pOnlineText->Get(ON_TEXT_WAIT));
		...
		case BARRACK_NOWWORK_BUY_END:
			Draw_Select(pSurface);

			m_NowWork        = BARRACK_NOWWORK_BUY_ENDPOOL;
			break;

		case BARRACK_NOWWORK_BUY_ENDPOOL:
			Draw_Select(pSurface);
			pMyOnlineWorld->pOnlineMsgBox->Draw(pSurface);
		...
		case BARRACK_NOWWORK_DISMISSAL_GETLIST:
			pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_NONE, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), pMyOnlineWorld->pOnlineText->Get(ON_TEXT_WAIT));
		...
		case BARRACK_NOWWORK_DISMISSAL_POOL:
			...
			if(pMyOnlineWorld->pOnlineMsgBox->IsActive() == FALSE)
			{
				...
				if((BExit.Process(TempbMouseDown) == TRUE) || LOBYTE(pMyOnlineWorld->pOnlineKeyboard->GetKeyStatus(VK_ESCAPE)))
				{
					m_NowWork = BARRACK_NOWWORK_DISMISSAL_END;
				}
				...
				{
					// ⑷營 Level煎 掘й熱 氈戲賊 OK
					m_NowWork = BARRACK_NOWWORK_DISMISSALCHECK_POOL;					
				}
...
END_HELP1:
		...
		case BARRACK_NOWWORK_DISMISSALCHECK_POOL:
...
/*				if(GetAsyncKeyState(VK_RETURN))
				{
					// Send Message
					pMyOnlineWorld->pOnlineClient->SendRequestBarrackDismissalSolder(m_DismissalSolderList[m_DismissalNowSelectSolder].siSlot);

					m_NowWork = BARRACK_NOWWORK_DISMISSALCHECK_POOL_RETURNWAIT;
				}
				else
				{
*/
				...
				else
				{
					...
					{
						m_NowWork = BARRACK_NOWWORK_DISMISSAL_POOL;
					}
				}
//				}
			}
		...
		case BARRACK_NOWWORK_DISMISSALCHECK_POOL_RETURNWAIT:
			pMyOnlineWorld->pOnlineMsgBox->SetMsg(ON_MSGBOX_TYPE_NONE, pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), pMyOnlineWorld->pOnlineText->Get(ON_TEXT_WAIT));
		...
		case BARRACK_NOWWORK_DISMISSAL_END:
			Draw_DismissalSelect(pSurface);

			m_NowWork        = BARRACK_NOWWORK_DISMISSAL_ENDPOOL;
			break;

		case BARRACK_NOWWORK_DISMISSAL_ENDPOOL:
...
//	撲貲	:	Draw Main
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
BOOL	OnlineVillageStructureBarrack::Draw_Select(LPDIRECTDRAWSURFACE7 pSurface)
{
	HDC				hDC;
	SI16			StartPos;
	SI32			TempCount;
...
//	m_Button_Main_Buy.SetX(m_Button_Main_Buy.GetX() - StartPos);
//	m_Button_Main_Dismissal.SetX(m_Button_Main_Dismissal.GetX() - StartPos);
	...
	if(clGrp.LockSurface(pSurface) == TRUE)
	{
		...
		pMyOnlineWorld->m_pPortrait->DrawAsNPC( m_siX + 15, m_siY + 65, NPC_KIND_BARRACK, pMyOnlineWorld->pOnlineVillage->GetNation() );
		
		// 堅辨ж晦 幗が 轎溘
		if(m_NowWork == BARRACK_NOWWORK_BUY_POOL) m_Button_Buy.Put(&m_Image_Button_All_EM, 0, 2, 1, BUTTON_PUT_NOMOVE);
		...
		for(TempCount1 = 0; TempCount1 < 6; TempCount1++)
		{
			for(TempCount2 = 0; TempCount2 < 2; TempCount2++)
			{
				...
				if(TempCount < m_MaxSelectSolder)
				{
					...
					{
						if((ptMouse.x >= TempXPos) && (ptMouse.x < (TempXPos + m_Image_CharacterSlotInterface->Header.Xsize)) && (ptMouse.y >= TempYPos) && (ptMouse.y < (TempYPos + m_Image_CharacterSlotInterface->Header.Ysize)))
						{
							if(bMouseDown)
							{
								clGrp.PutSpriteT(TempXPos, TempYPos, m_Image_CharacterSlotInterface->Header.Xsize, m_Image_CharacterSlotInterface->Header.Ysize, &m_Image_CharacterSlotInterface->Image[m_Image_CharacterSlotInterface->Header.Start[1]]);
...
//							clGrp.PutSpriteT(TempXPos, TempYPos, m_Image_CharacterSlotInterface->Header.Xsize, m_Image_CharacterSlotInterface->Header.Ysize, &m_Image_CharacterSlotInterface->Image[m_Image_CharacterSlotInterface->Header.Start[0]]);
					...
					// actdoll (2004/01/10 14:28) : 罹晦憮 橾欽 摹鷗脹 議葛攪檣陛 羹觼
					if((ptMouse.x >= TempXPos) && (ptMouse.x < (TempXPos + 60)) && (ptMouse.y >= TempYPos) && (ptMouse.y < (TempYPos + 68)))
					{
					...
					switch(pMyOnlineWorld->pOnlineVillage->GetNation())
					{
						case ON_COUNTRY_KOREA:
							...
							else
							{
								...
								if(CheckLevel(m_SolderList[TempCount].siLevel, m_SolderList[TempCount].siCredit) == FALSE)	siDrawFlag |= DF_FLAG_PORTRAIT_ICON_DIEOVER;
							}
	...
	if(pSurface->GetDC(&hDC) == DD_OK)
	{
		...
		// �こ羹� 旋濠 轎溘
		pMyOnlineWorld->pOnlineMegaText->DrawTextCenter(hDC, m_siX + 207, m_siY + 7,118,13,pMyOnlineWorld->pOnlineText->Get(ON_TEXT_BARRACK), RGB(200, 200, 200));
...
//	撲貲	:	Draw Main Check
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
BOOL	OnlineVillageStructureBarrack::Draw_Check(LPDIRECTDRAWSURFACE7 pSurface)
{
	HDC		hDC;
	BYTE	TempBuffer[256];
...
//		RenderCharacter(pSurface, (UI16)m_SolderList[m_NowSelectSolder].SolderInfo.uiKind, m_siCheckX + 25, m_siCheckY + 45, FALSE, 100, 0);
		//RenderCharacter(pSurface, (UI16)m_SolderList[m_NowSelectSolder].SolderInfo.uiKind, (UI16)(GetTickCount() / 33), 268, 251);
...
//	撲貲	:	Draw Dismissal
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
BOOL	OnlineVillageStructureBarrack::Draw_DismissalSelect(LPDIRECTDRAWSURFACE7 pSurface)
{
	HDC				hDC;
	SI16			StartPos;
	SI32			TempCount;
...
//	BExit.SetX(BExit.GetX() - StartPos);
//	m_Button_Dismissal.SetX(m_Button_Buy.GetX() - StartPos);
//	m_Button_Main_Buy.SetX(m_Button_Main_Buy.GetX() - StartPos);
//	m_Button_Main_Dismissal.SetX(m_Button_Main_Dismissal.GetX() - StartPos);
	...
	if(clGrp.LockSurface(pSurface) == TRUE)
	{
		...
		// п堅ж晦 幗が 轎溘
		if(m_NowWork == BARRACK_NOWWORK_DISMISSAL_POOL) m_Button_Dismissal.Put(&m_Image_Button_All_EM, 0, 2, 1, BUTTON_PUT_NOMOVE);
...
*/					
					//RenderCharacter(pSurface, pMyOnlineWorld->pMyData->GetFollowerCharID(m_DismissalSolderList[TempCount].siSlot), (UI16)(GetTickCount() / 33), TempXPos + (m_Image_CharacterSlotInterface->Header.Xsize / 2), TempYPos + (m_Image_CharacterSlotInterface->Header.Ysize / 2));
...
//					SI32	PrevExp	= pMyOnlineWorld->pMyData->GetFollowerParameterPrevExp(m_DismissalSolderList[TempCount].siSlot);
					...
					// actdoll (2004/01/10 14:15) : 蟾鼻鎰晦
					if(m_NowWork != BARRACK_NOWWORK_DISMISSAL_GETLIST)
					{
...
//							, (SI16)(((R32)CurLife / (R32)MaxLife) * 100.0f), (SI16)(((R32)(CurExp - PrevExp) / (R32)(NextExp - PrevExp)) * 100.0f));
		...
		// 釭陛晦 幗が 轎溘
		if(m_NowWork == BARRACK_NOWWORK_DISMISSAL_POOL) BExit.Put(&m_Image_Button_All_BM, 0, 2, 1, BUTTON_PUT_NOMOVE);
		else                                            BExit.Put(&m_Image_Button_All_BM, 0, 0, 0, BUTTON_PUT_NOMOVE);		
...
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
BOOL	OnlineVillageStructureBarrack::Draw_DismissalCheck(LPDIRECTDRAWSURFACE7 pSurface)
{
	HDC		hDC;
	BYTE	TempBuffer[256];
	BYTE	TempBuffer1[256];
...
//	撲貲	:	Check Level
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
BOOL	OnlineVillageStructureBarrack::CheckLevel(SI32 Lv, SI32 Credit)
{
...
//	撲貲	:	陝 辨煽滌煎 Text 雖薑
//收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收收
SI32	OnlineVillageStructureBarrack::GetFollowerBarrackText(UI16 Kind)
{
